...
export class AboutUs extends Component {
  ...
  componentDidMount(){
    axios.get('http://18.162.143.74/timeline')
      .then((data)=>{
        this.setState({
          timelines: data.data
        })
                        ...
                        <Row md={12} style={{marginTop: '20px'}}>
                          <Col md={{ order: index%2 ? 2 : 1, span:8}}
                            ...
                            <Image 
                              ...
                              src={timeline.image} rounded />
                          </Col>
                          <Col md={{ order: index%2 ? 1 : 2, span:4}} sm={12}>
                            <h1 className={index%2? "text-right": "text-left"}
                              style={{position: "absolute", bottom: 0, right: index%2 ? null : 15, fontSize: "3rem"}}>
                              {timeline.year}</h1>
                          </Col>
                        </Row>
